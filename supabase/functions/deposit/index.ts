...
// Initiates a deposit (user puts money INTO their MW wallet)
...
const MIN_DEPOSIT = 1000;   // UGX
const MAX_DEPOSIT = 5000000; // LivePay cap
...
serve(async (req) => {
  ...
  try {
    ...
    if (pending) {
      return json({ error: "You have a pending deposit. Please complete or cancel it first." }, 400);
    }

    const reference = `MWDEP${user.id.replace(/-/g, "").slice(0, 10)}${Date.now()}`.slice(0, 30);

    // Record pending transaction first (so webhook can find it)
    ...
    if (!lpRes.ok || lpData.success === false) {
      // Mark the tx as failed so user can retry
      await supabase
        .from("transactions")
